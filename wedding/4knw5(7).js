(function ($) {
    ...
    $.fn.fadingTexts = function (elem) {
        ...
        function init() {
            ...
            elem.hide();
            $parent.find('.fading-texts-container').css({
                'opacity': '0'
            });
            show_fading_texts()
        }
        ...
        function show_fading_texts() {
            ...
            $parent.find('.fading-texts-container').html(current_fading_text[current_fading_no]).animate({
                opacity: 1.0
            }, 500);
            setTimeout(function () {
                $parent.find('.fading-texts-container').animate({
                    opacity: 0
                }, 500, show_fading_texts)
            }, 3000)
        }
    };
    ...
    CATACORE.documentLoad = {
        init: function () {
            ...
            $('a[href^="#"]').on('click', function (e) {
                ...
                var curhash = this.hash,
                    ...
                    $curhash = $(curhash),
                    $target = $(target);
                if ($target.closest('.vc_tta').hasClass('cata-tta') || $target.parent().hasClass('vc_carousel-control')) {
                    if ($target.closest('.vc_tta').hasClass('cata-tta')) {
                        if ($('.cata-slick-slider').length > 0) {
                            var tabId = curhash.substr(1);
                            var elemSlick = $target.closest('.vc_tta').find('#' + tabId + ' .cata-slick-slider .slides');
        ...
        init_header: function () {
            ...
            $(window).on('scroll', function () {
                if (!(jQuery.browser.msie && jQuery.browser.version < 10)) {
                    ...
                    if (header_wrap.find('.page-header-wrap').data('parallax') == '1') {
                        ...
                        var coords = '50% ' + yPos + '%';
                        header_wrap.find('.page-header-wrap').css({
            ...
            $('.header-sidewidget > a').on('click', function (e) {
                e.preventDefault();
                $('#cata-template-wrapper').stop(true).transition({
                    x: '-300px'
                }, 700, 'easeInOutCubic');
        ...
        more_demo_aside: function () {
            $('body').on('init', '.more_demo_aside .la-tabs-wrapper', function () {
    ...
    CATACORE.pageSettings = {
        ...
        widget_init: function () {
            ...
            if ($selector.length > 0) {
                $selector.each(function (i, val) {
                    var pa = $(val);
                    pa.find('li:has(ul)').on('click', function (e) {
                        if (this == e.target) {
        ...
        settings: function ($element) {
            ...
            function isotopeFilter() {
                $element.find('.cata-isotope-filter li > a').on('click', function (e) {
            ...
            function calcHeightHorizontal(resize) {
                ...
                if (($(window).width() + scrollBarWidth) >= 768) {
                    $isotopItem.css({
                        height: $container.outerHeight(true) - (spacing * 2) - item_info_height
                    });
        ...
        progressBars: function () {
            ...
            $(selector).each(function () {
                $(this).appear(function () {
                    ...
                    $(this).find('.cata-bar-line').animate({
                        width: percentage + '%'
                    }, 1600);
                    if ($('body').hasClass('rtl')) {
                        $(this).find('.cata-percentage').animate({
                            right: percentage + '%'
                        }, 1600)
                    } else {
                        $(this).find('.cata-percentage').animate({
            ...
            if (typeof videojs == 'function') {
                ...
                if ($selector.length > 0) {
                    $selector.each(function (index, element) {
                        ...
                        player.pause();
                        $(this).find('.video-control').on('click', function (e) {
                            ...
                            var videoTime = setTimeout(function () {
                                ...
                                $elemVid.stop(false, true).animate({
                                    opacity: 1
                                }, 1000);
                                player.play()
                            }, 1000);
        ...
        google_maps: function () {
            ...
            if ($selector.length > 0) {
                ...
                $selector.each(function () {
                    ...
                    var map, mapOptions, isonce, mapStyle, mapStyle1, mapStyle2, mapStyle3, mapStyle4, mapStyle5, mapStyle6, mapStyle7, mapStyle8, mapStyle9, mapStyle10;
                    mapStyle1 = [{
...
jQuery(document).ready(function ($) {
    ...
    $(document).on('click', '.cata-category-list .cata-category-item', function () {
